...
from flask import Blueprint, jsonify, request,render_template, url_for, make_response, send_from_directory, current_app
from backend.application.models import db, User, ServiceProfessional, Service, Customer, Admin, BlockedUsers, ServiceRequest, Reviews, Role
...
import os
from .task import download_csv_report, monthly_report,daily_remainder
from backend.application.utils import cached, cached_with_args
import json
...
# @api.route('/api/csv_result/<task_id>')
# # @auth_required('token')
# # @roles_required('admin')
...
#         # return send_from_directory('/backend/static', filename, as_attachment=True, download_name=filename)
#          # Send the file directly with the proper content type
...
#     except Exception as e:
#         import traceback
#         print(f"Error retrieving CSV: {str(e)}")
#         print(traceback.format_exc())
#         return jsonify({'error': f'Error retrieving report: {str(e)}'}), 500

@api.route('/api/static/<path:filename>')
...
@cached(timeout=3600, key_prefix='service_types')  # Cache for 1 hour
...
##
        ...
        db.session.commit()
         ...
         # print(service_request.professional_id.professional_name)
        # result = daily_remainder.delay(service_request.professional_id)
# ####
        # Only send notification when status actually changes
        if previous_status != new_status:
            print(f"Triggering daily_remainder with service_request.id={service_request.id} (type: {type(service_request.id)})")
            if service_request.id is not None:
                 daily_remainder.delay(service_request.id)
                 print(f"Status change notification triggered for request {service_request.id}: {previous_status} → {new_status}")
            # else:
            #     print("ERROR: service_request.id is None, skipping Celery task execution.")
            # # Send the status change notification with the request ID
            # daily_remainder.delay(service_request.id)
            # print(f"Status change notification triggered for request {service_request.id}: {previous_status} → {new_status}")
#####
...
def submit_service_review(request_id):
    try:
        ...
        return jsonify({'error': f'Failed to submit review1: {str(e)}'}), 500
...
@api.route('/services/public', methods=['GET'])
@cached(timeout=300, key_prefix='public_services')
def get_public_services():
    try:
        ...
        print(f"Error fetching public services: {str(e)}")
        return jsonify({'error': 'Failed to fetch public services'}), 500
...
@auth_required()
@roles_required('admin')
@cached_with_args(timeout=180)  # Cache for 3 minutes
def get_admin_stats():
    try:
        # Get statistics
        total_users = User.query.filter(User.role.in_(['professional', 'customer'])).count()
        active_services = Service.query.filter_by(is_active=True).count()
        pending_approvals = ServiceProfessional.query.filter_by(is_approved=False).count()
...
def get_admin_activities():
    try:
        # For simplicity, return a list of recent activities
        ...
        print(f"Error fetching admin activities: {str(e)}")
        return jsonify({'error': 'Failed to fetch activities'}), 500

@api.route('/customer/change-password', methods=['PUT'])
@auth_required('token')
def change_customer_password():
    try:
        ...
        db.session.rollback()
        print(f"Error changing customer password: {str(e)}")
        return jsonify({'error': 'Failed to change password'}), 500
...
def search_services():
    ...
    try:
        ...
        if sort_by == 'price_asc':
            ...
            # This is more complex and would require a join with Reviews
            # We'll just do a basic ordering by name for now
        ...
        result = []
        for service in services:
            # Filtering by PIN code would require more complex logic with professionals
            # For now, we'll just include all services that match the other criteria
            service_data = {
                'id': service.id,
                'name': service.name,
                'description': service.description,
...
def get_professional_service_requests():
    """
    Get all service requests for the logged-in professional, with optional filtering
    """
    try:
        ...
        if current_user.role != 'professional':
            return jsonify({
                'success': False,
                'error': 'Unauthorized',
                'message': 'Only professionals can access this endpoint'
            }), 403
        
        # Get query parameters for filtering
        status = request.args.get('status', '')
        
        # Base query to get requests for this professional
        query = ServiceRequest.query.filter_by(professional_id=current_user.id)
        ...
        if status:
            if status == 'new':
                ...
                query = query.filter(ServiceRequest.status.in_(['accepted', 'in_progress']))
            elif status == 'completed':
                query = query.filter(ServiceRequest.status.in_(['completed', 'cancelled', 'rejected']))
            else:
                query = query.filter_by(status=status)
        
        # Execute query and get results
        service_requests = query.order_by(ServiceRequest.id.desc()).all()
        ...
        for req in service_requests:
            ...
            request_data = {
                ...
                'pin_code': req.pin_code or "",
                'price': req.price,
                'status': req.status,
                'notes': req.special_instructions or "",
        ...
        print(f"Error fetching professional requests: {str(e)}")
        return jsonify({
            'success': False,
...
@api.route('/professional/requests/<int:request_id>/status', methods=['PUT'])
@auth_required('token')
def update_professional_request_status(request_id):
    """
    ...
    try:
        ...
        # Ensure the request belongs to this professional
        if service_request.professional_id != current_user.id:
            return jsonify({
                'success': False,
                'error': 'Forbidden',
